/*
===============================================================================
 *  ESP32-S3 16-Channel Automatic Relay Smart Switch with Full MQTT Support
 *  Author: Raff Alds
...
void handleScheduleSet(uint8_t relayIndex, const char* jsonPayload) {
    ...
    for (JsonObject sch : schedules) {
        ...
        if (sch.containsKey("month_mask"))  
            relayConfigs[relayIndex].schedule.monthMask[s] = sch["month_mask"] | MONTH_ALL;
        s++;
    }    
    saveConfiguration();
    updateScheduleCache();
...
void startNTPRequest(const char* server) {
    ...
    ntpPacketBuffer[40] = (seconds >> 24) & 0xFF;
    ntpPacketBuffer[41] = (seconds >> 16) & 0xFF;
    ntpPacketBuffer[42] = (seconds >> 8) & 0xFF;
    ntpPacketBuffer[43] = seconds & 0xFF;
...
void processNTPResponse() {
    ...
    if (packetSize >= NTP_PACKET_SIZE) {
        ...
        if (mode == 4 && li != 3) {  
            uint32_t secs = (ntpPacketBuffer[40] << 24) |
...
void updateScheduleCache() {
    ...
    uint8_t yesterdayBit = (1 << ((ti->tm_wday + 6) % 7));    
    for (int i = 0; i < gpioConfig.count; i++) {
        if (relayConfigs[i].manualOverride) {
            scheduleActiveCache[i] = false;
            continue;
        ...
        for (int s = 0; s < 8; s++) {
            ...
            if (monthMask && !(monthMask & (1 << cachedMonth))) continue;            
            uint32_t monthDayMask = relayConfigs[i].schedule.monthDays[s];
            if (monthDayMask && !(monthDayMask & (1 << (cachedMonthDay - 1)))) continue;
            ...
            if (start == stop) {
                ...
                bool dayMatches = (relayConfigs[i].schedule.days[s] & cachedTodayBit) != 0;
                bool yesterdayMatches = (relayConfigs[i].schedule.days[s] & yesterdayBit) != 0;
                
                if ((cur >= start && dayMatches) || (cur < stop && yesterdayMatches)) {
                    hasActive = true;
                    break;
                }
            }
    ...
    for (int i = 0; i < gpioConfig.count; i++) {
        ...
        for (int s = 0; s < 8; s++) {
            ...
            if (start == stop) {
                ...
                uint8_t today = ti->tm_wday;
                uint8_t yesterday = (today + 6) % 7;
                if (cur >= start && (relayConfigs[i].schedule.days[s] & (1 << today))) {
                    shouldBeOn = true;
                    break;
                }
                if (cur < stop && (relayConfigs[i].schedule.days[s] & (1 << yesterday))) {
                    shouldBeOn = true;
                    break;
                }
            }
