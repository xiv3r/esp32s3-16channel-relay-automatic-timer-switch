...
void handleGetTime();
void handleBrowserTimeSync();  // FIXED: Removed duplicate
void handleGetWiFi();
void handleSaveWiFi();
...
struct tm* localtime64(uint64_t* timep) {
    int64_t totalOffset = (int64_t)sysConfig.gmt_offset + (int64_t)sysConfig.daylight_offset;
    int64_t rawTime = (int64_t)*timep;
    int64_t localTime = rawTime + totalOffset;
    if (localTime < 0) {
        localTime = 0;
    }    
    uint64_t safeLocalTime = (uint64_t)localTime;
    return gmtime64(&safeLocalTime);
}
...
inline uint64_t getLocalEpoch(uint64_t utcEpoch) {
    ...
    int64_t epoch = (int64_t)utcEpoch;
    int64_t localEpoch = epoch + offset;
    if (localEpoch < 0) {
        localEpoch = 0;
    }    
    return (uint64_t)localEpoch;
}
