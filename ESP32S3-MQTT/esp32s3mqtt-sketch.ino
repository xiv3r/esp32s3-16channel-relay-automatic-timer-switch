/*
===============================================================================
 *  ESP32S3 16-Channel Automatic Relay Smart Switch with Full MQTT Support
 *  Author: Raff Alds
...
// =============================================================================
//  64-bit GMT Time Implementations 
// =============================================================================
...
// =============================================================================
//  64-bit Localtime Implementations 
// =============================================================================
...
void publishHADiscovery() {
    ...
    if (deviceName.isEmpty()) deviceName = "ESP32S3_Relay16";    
    for (int i = 0; i < getActiveRelayCount(); i++) {
...
void publishSchedule(uint8_t relayIndex) {
    ...
    for (int s = 0; s < 8; s++) {
        ...
        sch["month_mask"] = relayConfigs[relayIndex].schedule.monthMask[s];  
    }    
    String payload;
    serializeJson(doc, payload);
...
void handleScheduleSet(uint8_t relayIndex, const char* jsonPayload) {
    ...
    for (JsonObject sch : schedules) {
        ...
        if (sch.containsKey("month_mask")) {
            uint16_t rawMonthMask = sch["month_mask"] | 0;
            relayConfigs[relayIndex].schedule.monthMask[s] = (rawMonthMask == 0) ? MONTH_ALL : rawMonthMask;
        }
        s++;
    }
    saveConfiguration();
...
void loadMqttFullConfig() {
    ...
    if (len != sizeof(MqttFullConfig) || mqttFullConfig.magic != MQTT_FULL_MAGIC) {
        ...
        snprintf(mqttFullConfig.client_id, 40, "ESP32S3_Relay16_%06X", (uint32_t)(ESP.getEfuseMac() & 0xFFFFFF));
...
<small>When disabled, ESP32S3 runs in AP-only mode (no internet/NTP). Relay schedules using internal RTC/DS3231 will still work.</small>
</div>
<hr>
...
function loadWiFiStatus(){
  fetch('/api/wifi').then(r=>r.json()).then(d=>{
    ...
    else{s.innerHTML='<span style="font-weight:700">\u26D4 WiFi Station Disabled</span><br>ESP32S3 running in AP-only mode.';s.className='alert aw'}
  }).catch(()=>{});
}
...
setInterval(tick,1000);tick();
fetch('/api/ntp').then(r=>r.json()).then(d=>{document.getElementById('srv').value=d.ntpServer||'time.google.com';document.getElementById('gmt').value=d.gmtOffset||28800;document.getElementById('dst').value=d.daylightOffset||0;document.getElementById('shi').value=d.syncHours||1}).catch(()=>{});
function save(){
...
<div class="fg"><label>Client ID</label><input type="text" id="clientId" placeholder="ESP32S3_Relay16" required></div>
...
<small>Automatically creates entities in Home Assistant via MQTT Discovery protocol.</small>
</div>
